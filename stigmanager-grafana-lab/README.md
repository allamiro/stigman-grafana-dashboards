...
| stigman-exporter | built from `metrics-history/exporter` | 1.2.0 | Python 3.12 |
...
## 10z. Cyber analyst dashboards (`grafana/dashboards-cyber/`)

Folder **STIG Posture (Cyber Analysis)** — built for analysts, not managers
(generated by `scripts/update-cyber-dashboards.py`):

* **STIG Posture — Cyber Drilldown** (`/d/stig-cyber-drilldown`): pick a
  collection and pivot posture by **STIG benchmark**, **asset**, and
  **label** (site/OS/department), with per-row coverage, CAT 1/2/3 counts,
  workflow statuses and CORA risk — every column filterable — plus a
  **Top failing rules** hit list (findings aggregated by rule, ranked by how
  many assets fail each rule).
* **STIG Posture — Cyber Asset Deep Dive** (`/d/stig-cyber-asset`):
  collection + asset dropdowns; the asset's posture donut, coverage, risk,
  workflow status, per-STIG breakdown and its complete failing-rule punch
  list.

Severity and workflow-status colors match the native STIG Manager UI
(sampled from the client's `stigman.css`): CAT 1 `#EBA794`, CAT 2 `#FFD68F`,
CAT 3 `#CDD2EA`; Saved periwinkle, Submitted `#C3DEAB`, Accepted `#81DFAA`,
Rejected `#EBA995`, Unassessed `#EDEDED`. The same palette is used across
every dashboard in the lab.

Demo data for these dashboards comes from `./scripts/seed-cyber-data.sh`,
which adds two more collections — **Network Infrastructure** (VPN SRG) and
**Corporate Workstations** (Windows 10) — with collection labels applied to
assets and the full workflow-status mix (saved, submitted, accepted and
rejected reviews). It also labels the two original collections and grants
the reporter read-only access to the new ones.
...
| Docker standalone | `./metrics-history/docker/run-docker.sh` (builds `stigman-exporter:1.2.0`, `docker run -d`, port 9633) |
