...
| *(alternative)* exporter container image | `docker build -t stigman-exporter:1.2.0 metrics-history/exporter && docker save stigman-exporter:1.2.0 -o stigman-exporter-1.2.0.tar` |
...
```bash
docker load -i stigman-exporter-1.2.0.tar
docker run -d --name stigman-exporter --restart unless-stopped -p 9633:9633 \
  --env-file /etc/stigman-exporter.env stigman-exporter:1.2.0
```
